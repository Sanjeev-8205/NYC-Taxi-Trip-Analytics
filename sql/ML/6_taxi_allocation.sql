-- 6_taxi_allocation.sql
-- Purpose: To allocate the taxis for the most efficient zone × hours.
-- Source: mart.fct_zone_hourly_efficiency,  ML.hourly_taxi_demand_predictions
--Note:
-- -The zone clusters like high demand,efficiency-driven are derived from the pickup_zone_name only.
-- -The priority hours are derived from the pickup_zone_name x pickup_hour.
-- -The predicted trip is used only during taxi allocation.

CREATE OR REPLACE TABLE `nyc-taxi-485918.ML.taxi_allocation` AS
...
eligible_zone_hours AS(
  ...
  WHERE c.zone_clusters IN('Efficiency-driven','High-demand High-efficiency') AND p.hour_priority='High Priority'
),
eligible_with_predictions AS (
  SELECT
    pickup_zone_name,
    zone_clusters,
    predicted_hour,
  ...
  FROM (
    SELECT
      ...
      zh.hour_priority,
      ROW_NUMBER() OVER (
        PARTITION BY zh.pickup_zone_name, p.predicted_hour
        ORDER BY p.predicted_trips DESC
      ) AS rn
    FROM eligible_zone_hours zh
    JOIN analytics.hourly_taxi_demand_predictions p
      ON zh.pickup_zone_name = p.pickup_zone_name
     AND zh.pickup_hour = p.predicted_hour
  )
  WHERE rn = 1
)
SELECT
...
FROM eligible_with_predictions
ORDER BY pickup_zone_name, allocated_taxis DESC;
